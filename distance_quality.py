...
row = [img_num, df.loc[df['Image'] == int(
    img_num), 'Label'].values[0]] + [features[k] for k in df.columns[2:]]
df = df.append(pd.Series(row, index=df.columns), ignore_index=True)

# predict future values using KNN and KMeans
X = df.iloc[:, 1:].values
...
df['Percentage'] = np.round(
    (1 - distances / np.max(distances, axis=0)) * 100, 2)
